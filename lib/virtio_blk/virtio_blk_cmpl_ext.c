...
#define T(name, flags)[flags] = virtio_blk_process_compl_ext_##name,
	VIRTIO_BLK_COMPL_FASTPATH_MODES
#undef T
};

static __rte_always_inline void
process_enq_compl(struct virtio_blk_queue *q, struct dao_dma_vchan_state *mem2dev,
...
push_compl_ext_data(struct virtio_blk_queue *q, struct dao_dma_vchan_state *mem2dev, void **vbufs,
...
static __rte_always_inline int
virtio_blk_process_compl_ext(struct virtio_blk_queue *q, void **vbufs, uint16_t nb_bufs,
				const uint16_t flags)
{
	...
	nb_compl = push_compl_ext_data(q, mem2dev, vbufs, nb_bufs, flags);

	return nb_compl;
}

#define T(name, flags)										\
	uint16_t virtio_blk_process_compl_ext_##name(void *q, void **vbufs, uint16_t nb_bufs)	\
	{											\
		return virtio_blk_process_compl_ext(q, vbufs, nb_bufs, (flags));		\
	}
