...
# RDMA RC Functional Tests
# Runs ibv_rc_pingpong in both directions over both octep RDMA data paths:
...
# ibv_rc_pingpong both ways EP_HOST <-> EP_REMOTE_HOST; endpoint vars from caller scope. $1=label.
function rc_pingpong_bidir()
{
	local label=$1
	local server_cmd client_cmd

	# Direction 1: EP_HOST as server, EP_REMOTE_HOST as client.
	echo "RC ($label): EP_HOST as Server -> EP_REMOTE_HOST as Client"
	server_cmd="bash -c \"${host_env} setsid ibv_rc_pingpong -g $host_gid_idx -d $host_rdma_dev -i 1 >/dev/null 2>&1 &\""
	client_cmd="bash -c \"${remote_host_env} ibv_rc_pingpong -g $remote_host_gid_idx -d $remote_host_rdma_dev -i 1 $host_ip\""
	ep_host_ssh_cmd "$server_cmd"
	sleep 1
	if ! $remote_host_ssh "$client_cmd"; then
		echo "RC ($label): EP_HOST-server direction FAILED"
		return 1
	fi

	# Direction 2: EP_REMOTE_HOST as server, EP_HOST as client.
	echo "RC ($label): EP_REMOTE_HOST as Server -> EP_HOST as Client"
	server_cmd="bash -c \"${remote_host_env} setsid ibv_rc_pingpong -g $remote_host_gid_idx -d $remote_host_rdma_dev -i 1 >/dev/null 2>&1 &\""
	client_cmd="bash -c \"${host_env} ibv_rc_pingpong -g $host_gid_idx -d $host_rdma_dev -i 1 $remote_host_ip\""
	$remote_host_ssh "$server_cmd"
	sleep 1
	if ! ep_host_ssh_cmd "$client_cmd"; then
		echo "RC ($label): EP_REMOTE_HOST-server direction FAILED"
		return 1
	fi

	echo "RC ($label): PASSED (both directions)"
	return 0
}

# Function to run RC functional tests
function rdma_rc_ping()
{
	...
	if [[ "${RDMA_SCENARIO:-both}" != "mlx" && -n "${EP_REMOTE:-}" ]]; then
		...
		echo "[O<->O] step 4/4: run ibv_rc_pingpong (both directions)"
		rc_pingpong_bidir "Octeon<->Octeon" || overall=1
		rdma_tests_cleanup
		ran=1
	fi
	...
	if [[ "${RDMA_SCENARIO:-both}" != "octeon" && -n "${EP_DEVICE_MLX_IFACE:-}" && -n "${EP_REMOTE_HOST_MLX_IFACE:-}" ]]; then
		...
		echo "[O<->MLX] step 4/4: run ibv_rc_pingpong (both directions)"
		rc_pingpong_bidir "Octeon<->Mellanox" || overall=1
		rdma_tests_cleanup
		ran=1
	fi
	...
	if [[ $overall -ne 0 ]]; then
		echo "RC Functional Tests FAILED"
		exit 1
	fi

	echo "RC Functional Tests completed successfully"
	return 0
}
test_run rdma_rc_ping 2
