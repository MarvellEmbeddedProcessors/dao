...
# This test verifies basic RC connectivity in both directions:
...
function rdma_rc_ping()
{
	...
	echo "RC Basic Connectivity Tests: Host as Server -> Remote as Client"

	local test_name="RC_FUNC_Host_Server_Remote_Client"
	local server_cmd="bash -c \"${host_env} setsid ibv_rc_pingpong -g $host_gid -d $host_rdma_dev -i 1 >/dev/null 2>&1 &\""
	local client_cmd="bash -c \"${remote_env} ibv_rc_pingpong -g $remote_gid -d $remote_rdma_dev -i 1 $host_ip\""
	...
	echo "RC Basic Connectivity Tests: Remote as Server -> Host as Client"

	test_name="RC_FUNC_Remote_Server_Host_Client"
	server_cmd="bash -c \"${remote_env} setsid ibv_rc_pingpong -g $remote_gid -d $remote_rdma_dev -i 1 > /dev/null 2>&1 &\""
	client_cmd="bash -c \"${host_env} ibv_rc_pingpong -g $host_gid -d $host_rdma_dev -i 1 $remote_ip\""
