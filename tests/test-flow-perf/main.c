...
static const struct option_dict {
	...
	{.str = "icmpv6",
	 ...
	 .map = &flow_items[0],
	 .map_idx = &items_idx},
	{.str = "ingress", .mask = INGRESS, .map = &flow_attrs[0], .map_idx = &attrs_idx},
	{.str = "egress", .mask = EGRESS, .map = &flow_attrs[0], .map_idx = &attrs_idx},
	{.str = "transfer", .mask = TRANSFER, .map = &flow_attrs[0], .map_idx = &attrs_idx},
	{.str = "port-id",
		...
		case 0:
			...
			if (strcmp(lgopts[opt_idx].name, "rules-batch") == 0) {
				...
				if (n > 0)
					...
					rte_exit(EXIT_FAILURE, "flow rules-batch should be > 0\n");
			}
...
create_meter_policy(void)
{
	...
	for (port_id = 0; port_id < nr_ports; port_id++) {
		...
		ret = rte_mtr_meter_policy_add(port_id, policy_id[port_id], &policy, &error);
		if (ret) {
...
meters_handler(int port_id, uint8_t core_id, uint8_t ops)
{
	...
	printf(":: Port %d :: Core %d Meter %s :: start @[%d] - end @[%d],"
	       ...
	       port_id, core_id, ops == METER_CREATE ? "create" : "delete", start_counter,
	       end_counter - 1, cpu_time_used, insertion_rate);
...
create_meter_profile(void)
{
	...
	for (port_id = 0; port_id < nr_ports; port_id++) {
		...
		ret = rte_mtr_meter_profile_add(port_id, DEFAULT_METER_PROF_ID, &mp, &error);
		if (ret != 0) {
...
lookup_flows(int port_id, uint8_t core_id)
{
	...
	tmbuf = rte_malloc(NULL, rules_batch * sizeof(struct rte_mbuf *), 0);
	if (tmbuf == NULL)
		rte_exit(EXIT_FAILURE, "Error: alloc tmbuf pointer failed.\n");
...
static inline void
destroy_flows(int port_id, uint8_t core_id, struct dao_flow **flows_list)
{
	...
	for (i = 0; i < (uint32_t)rules_count_per_core; i++) {
		...
		if (dao_flow_destroy(port_id, flows_list[i], &error)) {
...
insert_flows(int port_id, uint8_t core_id, uint16_t dst_port_id)
{
	...
	flows_list = rte_zmalloc("flows_list",
				 (sizeof(struct dao_flow *) * rules_count_per_core) + 1, 0);
	if (flows_list == NULL)
...
static inline void
do_tx(struct lcore_info *li, uint16_t cnt, uint16_t tx_port, uint16_t tx_queue)
{
	uint16_t nr_tx = 0;
	uint16_t i;
	...
	while (!force_quit) {
		...
		if (nr_valid_core > 1) {
			printf("%6s %'16" PRId64 " %'16" PRId64 " %'16" PRId64 "\n", "total",
			       total_tx_pkts, total_tx_drops, total_rx_pkts);
			nr_lines += 1;
		}
...
init_port(void)
{
	...
	mbuf_mp = rte_pktmbuf_pool_create("mbuf_pool", total_mbuf_num, mbuf_cache_size, 0,
					  mbuf_size, rte_socket_id());
	if (mbuf_mp == NULL)
		rte_exit(EXIT_FAILURE, "Error: can't init mbuf pool\n");
	...
	for (port_id = 0; port_id < nr_ports; port_id++) {
		...
		if (hairpin_queues_num != 0) {
			...
			for (hairpin_queue = tx_queues_count, std_queue = 0;
				...
				ret = rte_eth_tx_hairpin_queue_setup(port_id, hairpin_queue,
