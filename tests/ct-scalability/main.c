...
uint8_t pkt_ethipicmp_req[ETH_IP_ICMP_SZ] = {
	0x00, 0x0f, 0xb7, 0x06, 0x78, 0x30, 0x70, 0x35, 0x09, 0xde, 0xb4, 0xc9, 0x08, 0x00,
	0x45, 0x00, 0x00, 0x3c, 0x01, 0x9b, 0x00, 0x00, 0x74, 0x01, 0xca, 0x76, 0x0a, 0xc1,
	0x42, 0x77, 0x0a, 0x1c, 0x23, 0x5c, 0x08, 0x00, 0x4d, 0x2f, 0x00, 0x01, 0x00, 0x2c};

uint8_t pkt_ethipicmp_rep[ETH_IP_ICMP_SZ] = {
	0x70, 0x35, 0x09, 0xde, 0xb4, 0xc9, 0x00, 0x0f, 0xb7, 0x06, 0x78, 0x30, 0x08, 0x00,
	0x45, 0x00, 0x00, 0x3c, 0x37, 0x3c, 0x00, 0x00, 0x40, 0x01, 0xc8, 0xd5, 0x0a, 0x1c,
	0x23, 0x5c, 0x0a, 0xc1, 0x42, 0x77, 0x00, 0x00, 0x55, 0x2f, 0x00, 0x01, 0x00, 0x2c};
...
alloc_pkt_buffers(struct rte_mbuf ***pkts, uint32_t nb_lcores)
{
	...
	*pkts = rte_malloc(NULL, nr_buf * sizeof(struct rte_mbuf *), 0);
	...
	for (i = 0; i < nr_buf;) {
		...
		icmp->icmp_ident = i;
		icmp->icmp_cksum =
			icmp_checksum((uint16_t *)icmp, (sizeof(struct rte_icmp_hdr) + 32));
		ip->hdr_checksum = 0;
		ip->hdr_checksum = rte_ipv4_cksum(ip);

		(*pkts)[i + 1]->packet_type =
			RTE_PTYPE_L2_ETHER | RTE_PTYPE_L3_IPV4 | RTE_PTYPE_L4_ICMP;
		memcpy(rte_pktmbuf_mtod((*pkts)[i + 1], void *), &pkt_ethipicmp_rep[0],
		       ETH_IP_ICMP_SZ);
