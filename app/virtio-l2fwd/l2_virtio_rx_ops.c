...
l2_virtio_rx_ops_node_process_inline(struct rte_graph *graph, struct rte_node *node,
				     l2_virtio_rx_node_ctx_t *ctx)
{
	...
	if (q_count)
		rte_prefetch0(dao_virtio_netdevs[virtio_devid].qs[(queue << 1) + 1]);
	...
	while (q_count && nb_pkts < max_pkts) {
		...
		/* Dequeue from host using DMA ops mode */
		count = dao_virtio_net_dequeue_burst_ops(virtio_devid, virt_q, &mbufs[nb_pkts],
							 count);
		if (likely(count)) {
...
static __rte_always_inline uint16_t
l2_virtio_rx_ops_node_process(struct rte_graph *graph, struct rte_node *node, void **objs,
			      uint16_t cnt)
{
	...
	n_pkts = l2_virtio_rx_ops_node_process_inline(graph, node, ctx);
	return n_pkts;
}

static int
l2_virtio_rx_ops_node_init(const struct rte_graph *graph, struct rte_node *node)
{
	RTE_SET_USED(graph);
	RTE_SET_USED(node);
...
static struct rte_node_register l2_virtio_rx_ops_node_base = {
	.process = l2_virtio_rx_ops_node_process,
	.flags = RTE_NODE_SOURCE_F,
	.name = "l2_virtio_rx_ops",

	.init = l2_virtio_rx_ops_node_init,
...
l2_virtio_rx_ops_node_get(void)
{
	return &l2_virtio_rx_ops_node_base;
}

RTE_NODE_REGISTER(l2_virtio_rx_ops_node_base);
