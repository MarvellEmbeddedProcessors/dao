...
#define CA_COMP_DEV_CONSUMER_CORE_END   9
...
struct rte_mempool *ca_host_comp_dst_bufpool_get(uint8_t dev_id);
struct rte_mempool *ca_host_comp_op_mempool_get(uint8_t dev_id);
uint16_t ca_compdev_deq_noop(struct pending_queue *pq);
void host_dev_compress_pools_fini(uint8_t dev_id);
uint16_t ca_compdev_deq(struct pending_queue *pq);
int get_compdev_id(unsigned int core_id);
