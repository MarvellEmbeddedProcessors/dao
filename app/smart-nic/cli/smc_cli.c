...
cli_process(char *in, char *out, size_t out_size, __rte_unused void *obj)
{
	...
	if (rc == CMDLINE_PARSE_AMBIGUOUS) {
		len = snprintf(buf, BUFSIZ, MSG_CMD_FAIL, "Ambiguous command");
		len++;
		if (out_size < len)
			return;
		strncpy(out, buf, len);
	} else if (rc == CMDLINE_PARSE_NOMATCH) {
		len = snprintf(buf, BUFSIZ, MSG_CMD_FAIL, "Command mismatch");
		len++;
		if (out_size < len)
			return;
		strncpy(out, buf, len);
	} else if (rc == CMDLINE_PARSE_BAD_ARGS) {
		len = snprintf(buf, BUFSIZ, MSG_CMD_FAIL, "Bad arguments");
		len++;
		if (out_size < len)
			return;
		strncpy(out, buf, len);
