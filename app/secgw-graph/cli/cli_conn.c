...
#define SCLI_MSG_CMD_TOO_LONG "Command too long."

static int
scli_data_event_handle(struct scli_conn *conn, int fd_client)
{
	ssize_t len, i, rc = 0;
	...
	for (i = 0; i < len; i++) {
		if (conn->buf[i] == '\n') {
			...
			rc = write(fd_client, SCLI_MSG_CMD_TOO_LONG, strlen(SCLI_MSG_CMD_TOO_LONG));
			if (rc == -1)
				goto exit;
...
static int
scli_control_event_handle(struct scli_conn *conn, int fd_client)
{
	int rc;
...
struct scli_conn *
scli_conn_init(struct scli_conn_params *p)
{
	...
	/* Memory allocation */
	conn = calloc(1, sizeof(struct scli_conn));
	if (conn == NULL)
		goto exit;

	conn->welcome = calloc(1, SCLI_CONN_WELCOME_LEN_MAX + 1);
	conn->prompt = calloc(1, SCLI_CONN_PROMPT_LEN_MAX + 1);
	...
	if (setsockopt(fd_server, SOL_SOCKET, SO_REUSEADDR, (const char *)&reuse, sizeof(reuse)) <
	    0)
		goto free;

	rc = bind(fd_server, (struct sockaddr *)&server_address, sizeof(server_address));
	if (rc == -1) {
		dao_err("Failed to bind, err: %d (%s) ", errno, strerror(errno));
		goto free;
	}

	rc = listen(fd_server, 16);
	if (rc == -1) {
		dao_err("Failed to listen, err: %d (%s) ", errno, strerror(errno));
		goto free;
	}
	...
	/* Fill in */
	rte_strscpy(conn->welcome, p->welcome, SCLI_CONN_WELCOME_LEN_MAX);
	rte_strscpy(conn->prompt, p->prompt, SCLI_CONN_PROMPT_LEN_MAX);
...
void
scli_conn_free(struct scli_conn *conn)
{
	if (conn == NULL)
		return;
...
int
scli_conn_req_poll(struct scli_conn *conn)
{
...
int
scli_conn_msg_poll(struct scli_conn *conn)
{
	...
	if (event.events & EPOLLIN)
		rc_data = scli_data_event_handle(conn, fd_client);

	/* Control events */
	if (event.events & (EPOLLRDHUP | EPOLLERR | EPOLLHUP))
		rc_control = scli_control_event_handle(conn, fd_client);
